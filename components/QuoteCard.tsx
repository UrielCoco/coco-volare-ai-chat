...
type QuoteItem = { ref?: string; label?: string; qty?: number; unitPrice?: number; subtotal?: number; };
type QuoteTotals = { net?: number; tax?: number; fees?: number; discount?: number; grandTotal?: number; };
type QuoteValidity = { until?: string; notes?: string; };
...
function fmtMoney(n?: number, ccy?: string) {
  ...
  return new Intl.NumberFormat(getLocaleFromCurrency(ccy), {
    style: 'currency', currency: (ccy || 'USD').toUpperCase(), maximumFractionDigits: 2
  }).format(n);
}
const safe = (x?: string | number) => x == null ? '' : (typeof x === 'string' ? x.trim() : String(x));
    ...
    <div className="bp">
      ...
      <div className="hero">
        <div className="heroInner">
          <div className="brand">
            <img src="/images/logo-coco-volare.png" alt="Coco Volare" className="logo" draggable={false} />
        ...
        {/* Watermark sutil */}
        <div className="wm"><span className="wmLeft">CV</span><span className="wmArrow">✈︎</span><span className="wmRight">{ccy}</span></div>
      </div>

      {/* Perforación */}
      <div className="perforation" aria-hidden="true"><div className="dots" /></div>

      {/* 👇 LOGO CENTRADO Y GRANDE */}
      <div className="logoCenter">
        <img src="/images/logo-coco-volare.png" alt="Coco Volare" draggable={false} />
      </div>

      {/* Info rápida */}
      {(safe(data?.customer?.name) || safe(data?.meta?.createdAt) || safe(data?.customer?.email)) && (
        <div className="gridInfo">
          {safe(data?.customer?.name)   && <div className="info"><div className="label">Pasajero/Cliente</div><div className="value">{safe(data?.customer?.name)}</div></div>}
          {safe(data?.meta?.createdAt)  && <div className="info"><div className="label">Fecha</div><div className="value">{safe(data?.meta?.createdAt)}</div></div>}
          {safe(data?.customer?.email)  && <div className="info"><div className="label">Contacto</div><div className="value">{safe(data?.customer?.email)}</div></div>}
        </div>
      )}
      ...
      {(typeof totals.net === 'number' || hasTax || hasFees || hasDisc || showGrand) && (
        <>
          ...
          <div className="totals">
            {typeof totals.net === 'number' && <div className="trow"><span>Subtotal</span><span>{fmtMoney(totals.net, ccy)}</span></div>}
            {hasTax  && <div className="trow"><span>Impuestos</span><span>{fmtMoney(totals.tax!, ccy)}</span></div>}
            {hasFees && <div className="trow"><span>Cuotas/Fees</span><span>{fmtMoney(totals.fees!, ccy)}</span></div>}
            {hasDisc && <div className="trow"><span>Descuento</span><span>-{fmtMoney(Math.abs(totals.discount!), ccy)}</span></div>}
            {showGrand && <div className="trow grand"><span>Total</span><span>{fmtMoney(totals.grandTotal!, ccy)}</span></div>}
          </div>
        </>
      )}
      ...
      <style jsx>{`
        .bp { position: relative; width: 100%; background:#fff; color:#111; border-radius:22px; border:1px solid rgba(0,0,0,.06); box-shadow:0 6px 28px rgba(0,0,0,.10); overflow:hidden; }
        ...
        .wmLeft,.wmRight{ font-size:56px; line-height:1; } .wmArrow{ font-size:32px; }
        ...
        .perforation::before{ left:-11px; } .perforation::after{ right:-11px; }
        ...
        /* 👇 Nueva banda con logo centrado y grande */
        .logoCenter{ display:flex; justify-content:center; align-items:center; padding:14px 16px 2px; }
        .logoCenter img{ height:64px; width:auto; object-fit:contain; }

        .gridInfo{ display:grid; grid-template-columns:repeat(3,1fr); gap:12px; padding:12px 16px 4px; }
        .info .label{ font-size:11px; opacity:.65; } .info .value{ font-weight:700; }
        ...
        .right{ text-align:right; min-width:140px; } .right .unit{ font-size:12px; opacity:.8; } .right .sub{ font-weight:900; }

        .divider{ height:1px; background:rgba(0,0,0,.08); margin:8px 16px; }
        .totals{ padding:4px 16px 8px; display:grid; gap:6px; }
        .trow{ display:flex; justify-content:space-between; } .trow span:first-child{ opacity:.8; }
        .trow.grand{ padding-top:6px; border-top:1px dashed rgba(0,0,0,.22); font-size:16px; font-weight:900; }

        .notes{ padding:0 16px 14px; font-size:13px; } .notes .line + .line{ margin-top:6px; }
        ...
        @media (max-width:520px){ .gridInfo{ grid-template-columns:1fr; } .right{ min-width:120px; } }
        @media (prefers-color-scheme:dark){
          ...
          .divider{ background:rgba(255,255,255,.08); } .barcode{ opacity:.5; }
          .logoCenter img{ filter:brightness(1.05); }
        }
      `}</style>
    </div>
  );
